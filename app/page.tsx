  ...
  async function loadAll() {
    try {
      const [sysRes, ytRes, logsRes, workflowRes] = await Promise.all([
        ...
        fetch("/api/youtube/stats").catch(() => ({ json: () => ({}) })),
      ...
      <main style={{
        ...
        <section style={{ marginBottom: "48px" }}>
          <div style={{
            ...
            <h2 style={{ fontSize: "clamp(24px,4vw,36px)", marginBottom: "20px" }}>👑 CEO AI CHAT</h2>
            <div style={{
              background: "#020617",
              padding: "20px",
              borderRadius: "16px",
              ...
              {loading && <div style={{ color: "#38bdf8", fontSize: "14px" }}>🤖 CEO AI thinking...</div>}
            </div>
                ...
                tasks.map((task: any, i) => (
                  <div key={i} style={{
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: "12px", color: "#64748b" }}>{task.input?.slice(0, 50)}...</p>
                    </div>
                    <span style={{
                      fontSize: "11px",
                      fontWeight: "bold",
                      padding: "4px 10px",
                      borderRadius: "999px",
                      background: task.status === "completed" ? "#166534" : task.status === "running" ? "#1e3a5f" : task.status === "failed" ? "#7f1d1d" : "#1e293b",
                      color: task.status === "completed" ? "#22c55e" : task.status === "running" ? "#38bdf8" : task.status === "failed" ? "#ef4444" : "#94a3b8",
                    }}>
              ...
              {logs.length === 0 ? (
                ...
                logs.map((log: any, i) => (
                  <div key={i} style={{
                    ...
                    fontSize: "12px",
                    color: log.type === "ERROR" ? "#ef4444" : log.type === "SUCCESS" ? "#22c55e" : "#38bdf8",
                  }}>
                    <span style={{ color: "#475569" }}>{new Date(log.created_at).toLocaleTimeString()}</span> [{log.type}] {log.agent} — {log.message}
                  </div>
                ))
              )}
            </div>
        ...
        {/* AGENTS GRID */}
        <section style={{ marginBottom: "48px" }}>
          <h2 style={{ fontSize: "clamp(28px,5vw,48px)", marginBottom: "32px", fontWeight: "bold" }}>🧠 MULTI AI AGENTS</h2>
              ...
              <Link key={i} href={agent.href} style={{ textDecoration: "none" }}>
                <div style={{
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
                    ...
                    <span style={{ background: agent.color, padding: "4px 10px", borderRadius: "999px", fontSize: "11px", fontWeight: "bold", color: "#000" }}>{agent.status}</span>
                  </div>
        ...
        <section style={{ marginBottom: "48px" }}>
          <h2 style={{ fontSize: "clamp(24px,5vw,40px)", marginBottom: "24px", fontWeight: "bold" }}>🚀 YouTube AI Modules</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: "14px" }}>
            {[
              ...
              <Link key={i} href={item.href} style={{ textDecoration: "none" }}>
                <div style={{ background: item.color, padding: "18px 14px", borderRadius: "16px", textAlign: "center", fontWeight: "bold", fontSize: "15px", color: "white", cursor: "pointer" }}>
                  {item.label}
                </div>
              </Link>
            ))}
